\begin{frame}{Konstrukcija pravokotnice na premico $p$ skozi točko $T$}
	\begin{columns}
		\column{0.55\textwidth}
          \begin{itemize}
			 \item<1-7> Dani sta premica $p$ in točka $T$.
			 \item<2-7> Nariši lok $k$ s središčem v $T$.
			 \item<3-7> Premico $p$ seče v točkah $A$ in $B$.
			 \item<4-7> Nariši lok $m$ s središčem v $A$.
			 \item<5-7> Nariši lok $n$ s središčem v $B$ in z enakim polmerom.
			 \item<6-7> Loka se sečeta v točki $C$.
	...
	\begin{columns}
		...
		\column{0.45\textwidth}\centering
		 ...
		 % od presečišča premic; kot ATC je 45°.
				 ...
				 \pause
				 \coordinate (T') at ({-\td},{-\tt});
				 \draw[gray,thin] (T') arc[start angle=-60, end angle=60, radius=2] node[above left] {\scriptsize $m$};
				 \pause
				 \coordinate (T'') at ({\td},{-\tt});
				 \draw[gray,thin] (T'') arc[start angle=240, end angle=120, radius=2] node[above right] {\scriptsize $n$};
				 \pause
